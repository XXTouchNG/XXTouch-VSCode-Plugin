...
const outputChannel = vscode.window.createOutputChannel('touchelf');
let lastLog = '';
let interval = undefined;
function initDeviceView(context) {
    const deviceDataProvider = new DeviceDataProvider();
    vscode.window.registerTreeDataProvider('touchelf-device-view', deviceDataProvider);
    vscode.commands.registerCommand('touchelf.device.scan', () => {
    ...
    vscode.commands.registerCommand('touchelf.fetch.screen', (device) => __awaiter(this, void 0, void 0, function* () {
        const webviewPanel = vscode.window.createWebviewPanel('touchelf-fetch-screen', device ? `屏幕抓抓 - ${device.ip}` : '屏幕抓抓', vscode.ViewColumn.One, {
            enableScripts: true,
