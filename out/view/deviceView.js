...
function runScript(device_ip, shouldDebug) {
    var _a;
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const api = new api_1.API(device_ip);
...
function initDeviceView(context) {
    ...
    vscode.commands.registerCommand('xxtouch.device.remote-launch', (device) => {
        runScript(device.ip, false);
    });
    vscode.commands.registerCommand('xxtouch.device.remote-stop', (device) => {
        const api = new api_1.API(device.ip);
        api.stopScript().then((result) => {
            if (result)
                vscode.window.showInformationMessage('远程脚本已停止');
            else
                vscode.window.showErrorMessage('远程脚本停止失败');
        });
    });
    ...
    vscode.commands.registerCommand('xxtouch.device.nlog', (device) => {
        ...
        if (!terminal) {
            const highlightBinaryPath = context.asAbsolutePath(path.join('node_modules', 'cli-highlight', 'bin', 'highlight'));
