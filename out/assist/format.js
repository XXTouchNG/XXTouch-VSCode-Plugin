...
function initFormatter() {
    vscode.languages.registerDocumentFormattingEditProvider('lua', {
        provideDocumentFormattingEdits(document) {
            ...
            const enabled = configuration.get('touchelf.lua.format.enabled');
            if (!enabled) {
                return [];
            }
            ...
            try {
                formattedText = lua_fmt_ext_1.formatText(text, {
                    lineWidth: configuration.get('touchelf.lua.format.lineWidth'),
                    indentCount: configuration.get('editor.tabSize') || 4,
                    useTabs: false,
                    linebreakMultipleAssignments: false,
                    quotemark: configuration.get('touchelf.lua.format.singleQuote')
                        ? 'single'
                        : 'double',
